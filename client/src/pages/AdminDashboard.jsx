            ...
            <div>
              {posts.map((p) => (
                <div key={p.id} className="card">
                  ...
                  <div>{p.content}</div>
                  <div>By: {p.User ? p.User.username : "Unknown"}</div>
                  <button
                    style={{ marginTop: 8 }}
                    onClick={async () => {
              ...
              {editingId === s.id ? (
                ...
                <div>
                  ...
                  >
                    ...
                    <div
                      onDoubleClick={() => toggleOpen(s.id, s.open)}
                      style={{ cursor: "pointer" }}
                      title="Double-click to toggle open/closed"
                    >
                      {s.open ? "Open" : "Closed"}
                    </div>
                  </div>
                  ...
                  <div style={{ marginTop: 8 }}>
                    <button onClick={() => changeInventory(s.id, 1)}>
                      +1 Bike
                    </button>
                    <button
                      onClick={() => changeInventory(s.id, -1)}
                      style={{ marginLeft: 8 }}
                    >
                      -1 Bike
                    </button>
                    <button
                      onClick={() => {
                        const c = prompt("New capacity:", s.capacity);
                        if (c) setCapacity(s.id, Number(c));
                      }}
                      style={{ marginLeft: 8 }}
                    >
                      Set Capacity
